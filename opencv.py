...
class MyMainForm(QMainWindow, Ui_Form):
    ...
    def Q1_1(self):
        img = cv2.imread(
            './Dataset_opencvdl/Q1_Image/Uncle_Roger.jpg')  # read image
        cv2.namedWindow('1', 0)  # create a new window named '1'
        height = img.shape[0]
        width = img.shape[1]
        print('Height = ', height)
        print('Width = ', width)
        cv2.imshow('1', img)  # show the img in '1' window
        cv2.waitKey(0)  # push any key to next (blocking)
        # cv2.destroyAllWindows()
        cv2.destroyWindow('1')  # close named '1' window

    def Q1_2(self):
        # cv2.destroyAllWindows()
        img = cv2.imread('./Dataset_opencvdl/Q1_Image/Flower.jpg')
        (b, g, r) = cv2.split(img)  # split channels and stored to (b,g,r)
        zeros = np.zeros(img.shape[:2], dtype="uint8")  # create a zero array
        # because one channel is grayscale
    ...
    def Q1_3(self):
        ...
        flip = cv2.flip(img, 1)  # flip image
        # 1 - horizontal flip
        # 0 - vertical flip
    ...
    def Q1_4(self):
        ...
        def on_trackbar(val):
            ...
            # arg1 : first image
            # arg2 : weight of the first image
            # arg3 : second image
            # arg4 : weight of the second image
            # arg5 : scalar added to each sum == adjust brightness
    ...
    def Q2_1(self):
        ...
        median = cv2.medianBlur(img, 7)  # Noise Reduction
        # 用畫素點鄰域灰度值的中值來代替該畫素點的灰度值，讓周圍的畫素值接近真實的值從而消除孤立的噪聲點
    ...
    def Q2_2(self):
        img = cv2.imread('./Dataset_opencvdl/Q2_Image/Cat.png')
        # the more sigmaX & sigmaY higher the more blur
    ...
    def Q2_3(self):
        ...
        # arg3 : sigmaColor：色空間過濾器的sigma值，這個參數的值越大，表明該像素鄰域內有越寬廣的顏色會被混合到一起，產生較大的半相等顏色區域
        # arg4 : sigmaSpace：座標空間中濾波器的sigma值，如果該值較大，則意味着顏色相近的較遠的像素將相互影響，從而使更大的區域中足夠相似的顏色獲取相同的顏色。當d>0時，d指定了鄰域大小且與sigmaSpace五官，否則d正比於sigmaSpace.
    ...
    def Q3_1(self):
        ...
        gaussian_kernel = np.around(gaussian_kernel, 3)  # 四捨五入
        y, x = img.shape
        y = y - 3 + 1
        x = x - 3 + 1
        result = np.zeros((y, x))
    ...
    def Q3_2(self):
        ...
        for i in range(2, r, 1):
            for j in range(2, s, 1):
                x_result = abs(result[i-1][j+1] + 2 * result[i][j+1] + result[i+1]
                               [j+1] - result[i-1][j-1] - 2 * result[i][j-1] - result[i+1][j-1])
                if x_result > 255:
    ...
    def Q3_3(self):
        ...
        for i in range(2, r, 1):
            for j in range(2, s, 1):
                y_result = abs(result[i-1][j-1] + 2 * result[i-1][j] + result[i-1]
                               [j+1] - result[i+1][j-1] - 2 * result[i+1][j] - result[i+1][j+1])
                if y_result > 255:
    ...
    def Q4(self):
        ...
        rotation = cv2.getRotationMatrix2D(
            (160+int(Xnew), 84+int(Ynew)), float(angle), float(scale_val))
